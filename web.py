...
async def api_payment_init(req: Request, user: dict = Depends(get_current_user)):
    ...
    cursor.execute("SELECT payment_gateway, whatsapp_number FROM web_users WHERE role='ADMIN' LIMIT 1")
    admin_gw = cursor.fetchone()
    if admin_gw and admin_gw[0] == 'OFFLINE':
        wa_num = (admin_gw[1] or '').replace('+', '').replace(' ', '')
        txn_id = "TXN" + str(uuid.uuid4().hex)[:8].upper()
        now = get_ist_now().strftime("%Y-%m-%d %H:%M:%S")
        cursor.execute("INSERT INTO transactions (id, agent_email, plan_id, amount, status, created_at) VALUES (?, ?, ?, ?, 'PENDING_MANUAL', ?)", (txn_id, user['email'], plan_id, float(plan[1]), now))
        conn.commit(); conn.close()
        msg = f"Hi, I want to purchase plan: {plan[0]}\nAmount: ₹{plan[1]}\nOrder ID: {txn_id}\nEmail: {user['email']}"
        import urllib.parse
        wa_url = f"https://wa.me/{wa_num}?text={urllib.parse.quote(msg)}" if wa_num else "#"
        return {"status": "success_offline", "txn_id": txn_id, "whatsapp_url": wa_url, "amount": float(plan[1])}
